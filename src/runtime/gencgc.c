...
static page_index_t find_single_page(int WITH_TRACK(page_type), sword_t nbytes, generation_index_t gen)
{
    ...
    for ( ; page < page_table_pages ; ++page) {
        if (page_words_used(page) <= usage_allowance
            && (page_free_p(page) || page_extensible_p(page, gen, TR_PT_ARG(tr, page_type)))) return page;
    }
    ...
    for ( page = WITH_TRACK_INDEX(alloc_start_pages,tr)[page_type]; page < page_table_pages ; ++page) {
        if (page_words_used(page) < min_used && page_extensible_p(page, gen, TR_PT_ARG(tr, page_type)))
...
gc_alloc_new_region(sword_t nbytes, int WITH_TRACK(page_type), struct alloc_region *alloc_region, int unlock)
{
    ...
    if (page_type == PAGE_TYPE_CONS || page_type == PAGE_TYPE_SMALL_MIXED) {
        ...
        gc_dcheck(!unlock);
        page_index_t page;
        INSTRUMENTING(page = find_single_page(TR_PT_ARG(tr, page_type), nbytes, gc_alloc_generation),
    ...
    page_index_t first_page = get_alloc_start_page(TR_PT_ARG(tr, page_type)), last_page;
...
static void *new_region(struct alloc_region* region, sword_t nbytes, int WITH_TRACK(page_type))
{
    ...
    void* new_obj = gc_alloc_new_region(nbytes, TR_PT_ARG(tr, page_type), region, 0);
...
void *collector_alloc_fallback(struct alloc_region* region, sword_t nbytes, int WITH_TRACK(page_type))
{
    ...
    if (nbytes >= LARGE_OBJECT_SIZE) return gc_alloc_large(nbytes, TR_PT_ARG(tr, page_type));

    if (page_type != PAGE_TYPE_SMALL_MIXED) return new_region(region, nbytes, TR_PT_ARG(tr, page_type));
...
#define SMALL_MIXED_NBYTES_LIMIT (SMALL_MIXED_NWORDS_LIMIT * N_WORD_BYTES)
    ...
    if ((int)nbytes > (int)GENCGC_CARD_BYTES)
        return new_region(mixed_region, nbytes, TR_PT_ARG(tr, PAGE_TYPE_MIXED));
    if (!region->start_addr) { // region is not in an open state
        ...
        void* new_obj = gc_alloc_new_region(request, TR_PT_ARG(tr, page_type), region, 0);
...
{
    TR_PT_EXTRACT(new_tr, new_allocated)
