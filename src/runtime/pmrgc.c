...
static _Atomic struct allocator_state alloc_start_states[TRACKS_END][8]; // one for each combination (track, PAGE_TYPE_x)
...
void reset_alloc_start_pages(bool allow_free_pages) {
  for (int tr = 0; tr < TRACKS_END; tr++)
      for (int i = 0; i < 8; i++)
          alloc_start_states[tr][i] = (struct allocator_state){gencgc_alloc_start_page, allow_free_pages};
...
get_alloc_start_page(track_t track, unsigned int page_type)
{
    ...
    struct allocator_state global_start = alloc_start_states[track][page_type];
    page_index_t hint;
    switch (page_type) {
    case PAGE_TYPE_MIXED:
        if ((hint = thread_extra_data(th)->mixed_page_hint[track]) > 0 && hint <= global_start.page) {
            thread_extra_data(th)->mixed_page_hint[track] = - 1;
            return (struct allocator_state){hint, global_start.allow_free_pages};
        }
        break;
    case PAGE_TYPE_CONS:
        if ((hint = thread_extra_data(th)->cons_page_hint[track]) > 0 && hint <= global_start.page) {
...
static void
set_alloc_start_page(track_t track, unsigned int page_type, struct allocator_state state)
{
    if (page_type > 7) lose("bad page_type: %d", page_type);
    if (state.page > max_alloc_start_page) max_alloc_start_page = state.page;
    alloc_start_states[track][page_type] = state;
}
...
void *collector_alloc_fallback(struct alloc_region* region, sword_t nbytes, track_t track, int page_type) {
    struct allocator_state alloc_start = get_alloc_start_page(track, page_type);
    void *new_obj;
    if ((uword_t)nbytes >= (GENCGC_PAGE_BYTES / 4 * 3)) {
        uword_t largest_hole;
        page_index_t new_page = try_allocate_large(nbytes, track, page_type, gc_alloc_generation,
        ...
        bool success =
            try_allocate_small_from_pages(nbytes, region, track, page_type,
                                          gc_alloc_generation,
                                          &alloc_start, page_table_pages);
        if (!success) gc_heap_exhausted_error_or_lose(0, nbytes);
        new_obj = region->start_addr;
    }
    set_alloc_start_page(track, page_type, alloc_start);
    return new_obj;
}
...
# endif
    ...
    if (largep) {
        ...
        pre_search_for_small_space(nbytes, track, page_type, &alloc_start, page_table_pages);
        int __attribute__((unused)) ret = mutex_acquire(&free_pages_lock);
        gc_assert(ret);
