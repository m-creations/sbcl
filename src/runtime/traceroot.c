...
static lispobj mkcons(lispobj car, lispobj cdr)
{
    struct cons *cons = (struct cons*)
        gc_general_alloc(cons_region, sizeof(struct cons), DEFAULT_TRACK, PAGE_TYPE_CONS);
    cons->car = car;
    cons->cdr = cdr;
...
static lispobj make_sap(char* value)
{
    struct sap *sap = (struct sap*)
        gc_general_alloc(unboxed_region, sizeof(struct sap), DEFAULT_TRACK, PAGE_TYPE_UNBOXED);
